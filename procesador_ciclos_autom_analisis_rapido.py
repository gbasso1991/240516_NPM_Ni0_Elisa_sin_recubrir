...
if templog:
    try:
        ...
        plt.savefig(os.path.commonprefix(fnames_m)+'_templog.png',dpi=200,facecolor='w')
...
time=time_m[max(indx_in)+1:min(indx_out)]
...
#%% Todos los cilos c/ filtrado impar 
# cmap = mpl.colormaps['jet'] #'viridis'
# norm = plt.Normalize(temp_m.min(), temp_m.max())# Crear un rango de colores basado en las temperaturas y el cmap
...
plt.savefig(os.path.commonprefix(fnames_m)+'_ciclos_MH.png',dpi=300,facecolor='w')
# plt.show()
