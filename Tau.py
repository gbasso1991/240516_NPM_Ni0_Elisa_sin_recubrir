...
taus=np.array([ufloat(np.mean(tau_1_1),np.std(tau_1_1)),ufloat(np.mean(tau_1_2),np.std(tau_1_2)),ufloat(np.mean(tau_1_3),np.std(tau_1_3))])
SARs=np.array([ufloat(np.mean(SAR_1_1),np.std(SAR_1_1)),ufloat(np.mean(SAR_1_2),np.std(SAR_1_2)),ufloat(np.mean(SAR_1_3),np.std(SAR_1_3))])

for i,ar in enumerate(archivos_resultados):
    print('File:',ar,f'- tau: {taus[i]:.2e} s',f'- SAR: {SARs[i]:.1f} W/g')
ufloat(np.mean([t.nominal_value for t in taus]),np.std([t.nominal_value for t in taus]))

print(f'\nPromedio de las {len(taus)} medidas:')
tau1 = np.mean(unumpy.uarray([np.mean(tau_1_1),np.mean(tau_1_2),np.mean(tau_1_3)],[np.std(tau_1_1),np.std(tau_1_2),np.std(tau_1_3)]))
print(f' tau = {tau1} s')
SAR1 = ufloat(np.mean([S.nominal_value for S in SARs]),np.std([S.nominal_value for S in SARs]))
...
ax.text(0.95,0.1,rf'<$\tau$> = {tau1:.2ue} s',bbox=dict(alpha=0.8),transform=ax.transAxes,ha='right', va='bottom')
plt.legend()
plt.grid()
plt.ylabel(r'$\tau$ (s)')
plt.xlabel('Indx')
...
for i,fp in enumerate(filepaths):
    ...
    ax1.plot(H,M,label=f'{SARs[i]:1f} W/g')
...
taus=np.array([ufloat(np.mean(tau_4_1),np.std(tau_4_1)),ufloat(np.mean(tau_4_2),np.std(tau_4_2)),ufloat(np.mean(tau_4_3),np.std(tau_4_3))])
SARs=np.array([ufloat(np.mean(SAR_4_1),np.std(SAR_4_1)),ufloat(np.mean(SAR_4_2),np.std(SAR_4_2)),ufloat(np.mean(SAR_4_3),np.std(SAR_4_3))])
...
print(f' tau = {tau4} s')
SAR4 = ufloat(np.mean([S.nominal_value for S in SARs]),np.std([S.nominal_value for S in SARs]))
...
ax.text(0.95,0.1,rf'<$\tau$> = {tau4:.2ue} s',bbox=dict(alpha=0.8),transform=ax.transAxes,ha='right', va='bottom')
plt.legend()
plt.grid()
plt.ylabel(r'$\tau$ (s)')
plt.xlabel('Indx')
...
for i,fp in enumerate(filepaths):
    t,H,M,metadata=lector_ciclos(fp)
    ax2.plot(H,M,label=f'{SARs[i]:1f} W/g')
